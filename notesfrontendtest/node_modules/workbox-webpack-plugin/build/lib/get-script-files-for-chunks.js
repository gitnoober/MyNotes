...
module.exports = (compilation, chunkNames) => {
  ...
  for (const chunkName of chunkNames) {
    ...
    if (chunk) {
      ...
      compilation.warnings.push(new Error(`${chunkName} was provided to ` + `importScriptsViaChunks, but didn't match any named chunks.`));
    }
  }

  if (scriptFiles.size === 0) {
    compilation.warnings.push(new Error(`There were no assets matching ` + `importScriptsViaChunks: [${chunkNames}].`));
  }

...
class WheelFile(ZipFile):
    ...
    def open(self, name_or_info, mode="r", pwd=None):
        def _update_crc(newdata, eof=None):
            ...
            if eof and running_hash.digest() != expected_hash:
                raise WheelError(
                    "Hash mismatch for file '{}'".format(native(ef_name))
                )
        ...
        if (
            ...
            raise WheelError(
                "No hash found for file '{}'".format(native(ef_name))
            )
    ...
    def write_files(self, base_dir):
        logger.info(
            "creating '%s' and adding '%s' to it", self.filename, base_dir
        )
        deferred = []
    ...
    def close(self):
        ...
        if self.fp is not None and self.mode == "w" and self._file_hashes:
            ...
            zinfo = ZipInfo(
                native(self.record_path), date_time=get_zipinfo_datetime()
            )

...
def build_block_parser(md, **kwargs):
    ...
    parser.blockprocessors.register(
        HashHeaderProcessor(parser), "hashheader", 70
    )
    parser.blockprocessors.register(
        SetextHeaderProcessor(parser), "setextheader", 60
    )
    ...
    parser.blockprocessors.register(
        ReferenceProcessor(parser), "reference", 15
    )
    parser.blockprocessors.register(
        ParagraphProcessor(parser), "paragraph", 10
    )
    return parser
...
class BlockQuoteProcessor(BlockProcessor):
    ...
    def test(self, parent, block):
        return (
            bool(self.RE.search(block)) and not util.nearing_recursion_limit()
        )

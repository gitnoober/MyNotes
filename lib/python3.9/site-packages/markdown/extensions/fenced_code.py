...
class FencedCodeExtension(Extension):
    ...
    def extendMarkdown(self, md):
        ...
        md.preprocessors.register(
            FencedBlockPreprocessor(md, self.getConfigs()),
            "fenced_code_block",
            25,
        )
    ...
    def run(self, lines):
        ...
        while 1:
            ...
            if m:
                ...
                if m.group("attrs"):
                    id, classes, config = self.handle_attrs(
                        get_attrs(m.group("attrs"))
                    )
                    if len(classes):
                        lang = classes.pop(0)
                else:
                    ...
                    if m.group("hl_lines"):
                        ...
                        config["hl_lines"] = parse_hl_lines(
                            m.group("hl_lines")
                        )
                ...
                if (
                    ...
                    if lang:
                        lang_attr = ' class="{}{}"'.format(
                            self.config.get("lang_prefix", "language-"), lang
                        )
                    if classes:
    ...
    def handle_attrs(self, attrs):
        ...
        for k, v in attrs:
            if k == "id":
                ...
                configs[k] = parseBoolValue(
                    v, fail_on_errors=False, preserve_none=True
                )
            else:
                configs[k] = v

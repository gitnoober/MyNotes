...
def build_inlinepatterns(md, **kwargs):
    ...
    inlinePatterns.register(
        EscapeInlineProcessor(ESCAPE_RE, md), "escape", 180
    )
    inlinePatterns.register(
    ...
    inlinePatterns.register(
        ImageReferenceInlineProcessor(IMAGE_REFERENCE_RE, md),
        "image_reference",
        140,
    )
    inlinePatterns.register(
    ...
    inlinePatterns.register(
        AutolinkInlineProcessor(AUTOLINK_RE, md), "autolink", 120
    )
    inlinePatterns.register(
        AutomailInlineProcessor(AUTOMAIL_RE, md), "automail", 110
    )
    inlinePatterns.register(
...
class EscapeInlineProcessor(InlineProcessor):
    ...
    def handleMatch(self, m, data):
        ...
        if char in self.md.ESCAPED_CHARS:
            return (
                "{}{}{}".format(util.STX, ord(char), util.ETX),
                m.start(0),
                m.end(0),
            )
        else:
...
class BacktickInlineProcessor(InlineProcessor):
    ...
    def handleMatch(self, m, data):
        if m.group(3):
            ...
            return (
                m.group(1).replace("\\\\", self.ESCAPED_BSLASH),
                m.start(0),
                m.end(0),
            )
...
class AsteriskProcessor(InlineProcessor):
    ...
    PATTERNS = [
        EmStrongItem(
            re.compile(EM_STRONG_RE, re.DOTALL | re.UNICODE),
            "double",
            "strong,em",
        ),
        EmStrongItem(
            re.compile(STRONG_EM_RE, re.DOTALL | re.UNICODE),
            "double",
            "em,strong",
        ),
        EmStrongItem(
            re.compile(STRONG_EM3_RE, re.DOTALL | re.UNICODE),
            "double2",
            "strong,em",
        ),
        EmStrongItem(
                ...
                for index, item in enumerate(self.PATTERNS):
                    ...
                    if m:
                        ...
                        el = self.build_element(
                            m, item.builder, item.tags, index
                        )
                        parent.append(el)
                        last = el
...
class UnderscoreProcessor(AsteriskProcessor):
    ...
    PATTERNS = [
        EmStrongItem(
            re.compile(EM_STRONG2_RE, re.DOTALL | re.UNICODE),
            "double",
            "strong,em",
        ),
        EmStrongItem(
            re.compile(STRONG_EM2_RE, re.DOTALL | re.UNICODE),
            "double",
            "em,strong",
        ),
        EmStrongItem(
        ...
        EmStrongItem(
            re.compile(SMART_STRONG_RE, re.DOTALL | re.UNICODE),
            "single",
            "strong",
        ),
        EmStrongItem(
            re.compile(SMART_EMPHASIS_RE, re.DOTALL | re.UNICODE),
            "single",
            "em",
        ),
    ]
...
class LinkInlineProcessor(InlineProcessor):
    ...
    def getLink(self, data, index):
            ...
            for pos in range(index, len(data)):
                ...
                if bracket_count == 0:
                    ...
                    if exit_quote >= 0 and quote == last:
                        ...
                        title = "".join(
                            data[start_alt_quote : exit_alt_quote - 1]
                        )
                    else:
        ...
        if title is not None:
            title = self.RE_TITLE_CLEAN.sub(
                " ", dequote(self.unescape(title.strip()))
            )
